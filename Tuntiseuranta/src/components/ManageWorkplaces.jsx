...
const ManageWorkplaces = () => {
    ...
    const [eveningMultiplier, setEveningMultiplier] = useState('');
    ...
    const handleAddWorkplace = async () => {
        ...
        if (!workplace || !salary) {
            alert("Please fill in workplace name, salary, and multipliers.");
            return;
        }
        ...
        try {
            await addDoc(collection(db, "Workplaces"), {
                ...
                multipliers: {
                    night: parseFloat(nightMultiplier) || 1, // Default multiplier is 1
                    evening: parseFloat(eveningMultiplier) || 1,
                    holiday: parseFloat(holidayMultiplier) || 1,
                }
            });
    ...
    const handleDeleteWorkplace = async (id) => {
        if (!isAdmin) {
            alert('Only admins can delete workplaces.');
            return;
        }
        ...
        >
            <Paper elevation={3} sx={{ padding: 4, maxWidth: 600, width: '100%' }}>
                ...
                <Box component="form" sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    />
                    <TextField 
                        label="Evening Shift Multiplier" 
                        variant="outlined" 
                        fullWidth 
                        value={eveningMultiplier}
                        onChange={(e) => setEveningMultiplier(e.target.value)}
                    />
                    <TextField 
                        label="Holiday Shift Multiplier" 
                        variant="outlined" 
                        fullWidth 
                                ...
                                secondary={
                                    <>
                                        <Typography variant="body2">
                                            Base Salary: €{workplace.salary?.toFixed(2) || 'N/A'} per hour
                                        </Typography>
                                        <Typography variant="body2">
                                            Night Shift Multiplier: {workplace.multipliers?.night || 1}
                                        </Typography>
                                        <Typography variant="body2">
                                            Evening Shift Multiplier: {workplace.multipliers?.evening || 1}
                                        </Typography>
                                        <Typography variant="body2">
                                            Holiday Shift Multiplier: {workplace.multipliers?.holiday || 1}
                                        </Typography>
                                    </>
                                }
                            />
